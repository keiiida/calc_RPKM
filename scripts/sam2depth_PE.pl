...
#For PE MODE
#If The data is fr-second strand, modify here
my %dir_hash = ();
if($ARGV[6] =~ /^stranded$/){
   %dir_hash = (
      "F"  => "Plus",
...
close(F);
chomp($tt);
my $READS = $tt - $unmapped; #READS ments Fragment, here

#Check multiple mapping
my %nh_mem = ();
open(F,"samtools view -f 0x140 $f|") || die "Cannot open the file; $f, with samtools\n";
while(<F>){
   $nh_mem{ (split(/\t/,$_))[0] }++;
}
close(F);
...
foreach my $chr (@c_box){
   ...
   while(<F>){
      ...
      my $dir0 = "";
      #For PE MODE
      if( ( substr($bb,5,1) == 1 && substr($bb,-5,1) == 1 ) || #R1 
          ( substr($bb,4,1) == 1 && substr($bb,-5,1) == 0 )){  #R2
         $dir0 = "F";
         #Treated as “fr-firststrand
      }elsif( ( substr($bb,5,1) == 1 && substr($bb,-5,1) == 0 ) ||
              ( substr($bb,4,1) == 1 && substr($bb,-5,1) == 1 )){
         $dir0 = "R";
      }else{
         push(@bad_box,join("\t",@tmp));
      }
      $dir = $dir_hash{$dir0};
      if($dir eq ""){
         next;
...
if(@bad_box > 0){
   my $log_file2 = $log_file . ".Additonal_log_for_Unkown_Direction";
   open(F,"> $log_file2") || die "Cannot open the file; $log_file2 to write\n";
   printf F "#These alignment is ignored due to unknown direction types.\n";
   printf F "$_\n",join("\n",@bad_box);
   close(F);
}
